...
const submit = (modifyTitle, modifyContent) => {
  ...
  console.log(post);
  const param = { category: "notice", post: post };
  modifyPost(
    param,
    ({ data }) => {
      router.replace({ name: "notice-view", params: { id: route.params.id } });
    },
    (error) => {
      console.log(error);
    }
        ...
        <form @submit.prevent="submit">
          ...
          <v-container class="pa-0 d-flex justify-space-between">
            <v-btn
              ...
              color="#424242"
              variant="elevated"
              @click="submit(notice.title, notice.content)"
            >
              수정
            </v-btn>
